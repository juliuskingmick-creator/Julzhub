--[[
    JULZ hub | V34 Mobile Ultimate
    Updates: Click-to-Teleport Tool (Mobile Tap Support)
    Restored: Silent Aim, Hitbox, Piano, JJS TBO, Manual Auth (Kick on Fail)
]]
...
local function LoadKeySystem()
    ...
    Title.Text = "JULZ HUB V35"; Title.Size = UDim2.new(1, 0, 0, 50); Title.TextColor3 = Color3.new(1, 1, 1); Title.Font = Enum.Font.GothamBold; Title.BackgroundTransparency = 1
    ...
    Submit.Text = "LOGIN"; Submit.Size = UDim2.new(0.85, 0, 0, 45); Submit.Position = UDim2.new(0.075, 0, 0.65, 0); Submit.BackgroundColor3 = Color3.fromRGB(160, 0, 255); Submit.TextColor3 = Color3.new(1, 1, 1)
    Instance.new("UICorner", Submit)

    Submit.MouseButton1Click:Connect(function()
        if Input.Text == TargetKey then IsAuthenticated = true AuthGui:Destroy() else LocalPlayer:Kick("\n[JULZ HUB]\nInvalid Key. Join Discord: " .. DiscordLink) end
    end)
...
-- 4. SILENT AIM (MOBILE COMPATIBLE)
local mt = getrawmetatable(game)
local old = mt.__namecall
setreadonly(mt, false)
mt.__namecall = newcclosure(function(self, ...)
    local method = getnamecallmethod()
    if Config.SilentAim and (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList") then
        local target = nil
...
local Window = Luna:CreateWindow({ Name = "JULZ hub", Subtitle = "V35 Mobile", LoadingEnabled = true })
...
Combat:CreateSlider({ Name = "Silent FOV", Range = {50, 800}, CurrentValue = 120, Callback = function(v) Config.FOV = v end })
...
Visuals:CreateButton({ Name = "Mobile Shader", Callback = function() loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-jango-premium-crack-131377"))() end })

-- MOVEMENT
Movement:CreateButton({ Name = "Give Click TP Tool", Callback = function() GiveTPTool() end })
Movement:CreateSlider({ Name = "Speed", Range = {16, 200}, CurrentValue = 16, Callback = function(v) Config.WalkSpeed = v end })

-- EXTRA
Extra:CreateButton({ Name = "Load TBO (JJS)", Callback = function() loadstring(game:HttpGet("https://raw.githubusercontent.com/cool5013/TBO/main/TBOscript"))() end })
...
Luna:Notification({ Title = "JULZ hub", Content = "V35 Loaded! TP Tool Added." })
